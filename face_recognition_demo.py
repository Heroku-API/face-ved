...
def build_argparser(src):
    parser = ArgumentParser()
    input = src
    input=0
    model_face_detection =r"model_files\face-detection-0200.xml"
    ...
    general.add_argument('-i', '--input', default=input,required=False,help='Required. An input to process. The input must be a single image, '
    ...
    gallery.add_argument('-fg', default='faces', help='Optional. Path to the face images directory.')
    gallery.add_argument('--run_detector', action='store_true',default=True,
    ...
    models.add_argument('-m_fd', type=Path, default= model_face_detection,required=False,
                        help='Required. Path to an .xml file with Face Detection model.')
    models.add_argument('-m_lm', type=Path, default=landmarks_model,required=False,
                        help='Required. Path to an .xml file with Facial Landmarks Detection model.')
    models.add_argument('-m_reid', type=Path, default=model_refinder,required=False,
    ...
    infer.add_argument('-t_id', metavar='[0..1]', type=float, default=0.4,
...
def draw_detections(frame, frame_processor, detections, output_transform,camera_ip,person_name):
    if camera_ip==webcam:
        cam_name='webcam'
    else:
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        if name[0]==person_name:
            ...
            text=f"person found in {cam_name}"
            print(text)
            cv2.putText(frame, text, (350,50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 1)
        # if text !='unknown':
...
def main(src,cam_name):

    args = build_argparser(src).parse_args()
    print(args.input)
    camera_ip=args.input

    cap = cv2.VideoCapture(args.input)
    #cap=cv2.VideoCapture('rtsp://admin:Admin@123@192.168.29.65:554/Streaming/Channels/1')
    ...
    while True:
        ...
        frame = draw_detections(frame, frame_processor, detections, output_transform,camera_ip,person_name)
        ...
        if not args.no_show:
            cv2.namedWindow(cam_name, cv2.WINDOW_FULLSCREEN)
...
if __name__ == '__main__':
    # camera_details={"rtsp://admin:Admin123@192.168.29.64:554/Streaming/Channels/1":"cam1",
    #                 "rtsp://admin:Admin123@192.168.29.65:554/Streaming/Channels/1":"cam2"}

    #front_camera="rtsp://admin:Admin123@192.168.29.64:554/Streaming/Channels/1"
    webcam=0
    front_camera="rtsp://admin:Admin123@192.168.29.65:554/Streaming/Channels/1"
    person_name = input('Enter the person to want to want')
    t3 = Thread(target=main, args=[0, "webcam", ])

    #t3 = Thread(target=main, args=["rtsp://admin:Admin123@192.168.29.64:554/Streaming/Channels/1", "cam 3", ])
    t3.start()
    t4 = Thread(target=main, args=["rtsp://admin:Admin123@192.168.29.65:554/Streaming/Channels/1", "cam 4", ])
    t4.start()
    t3.join()
    t4.join()
    ...
    sys.exit(main(src=None,cam_name=None) or 0)
    #sys.exit(main() or 0)
